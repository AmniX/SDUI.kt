 ...
 */
data class ValidationResult(val isValid: Boolean, val issues: List<ValidationIssue> = emptyList()) {
    companion object {
...
object SduiValidator {
    ...
    private fun isValidDimension(value: String): Boolean = when {
        value == "100%" -> true
        value.endsWith("%") -> {
    ...
    private fun isValidFontWeight(fontWeight: String): Boolean = when (fontWeight.lowercase()) {
        "normal", "bold", "light", "medium", "thin", "ultralight", "semibold", "extrabold", "black" -> true
        else -> false
    }

    private fun isValidAlignment(alignment: String): Boolean = when (alignment.lowercase()) {
        "start", "end", "center", "justify", "left", "right" -> true
        else -> false
    }

    private fun isValidFlexDirection(flexDirection: String): Boolean = when (flexDirection.lowercase()) {
        "row", "column", "row-reverse", "column-reverse" -> true
        else -> false
    }

    private fun isValidJustifyContent(justifyContent: String): Boolean = when (justifyContent.lowercase()) {
        "start", "end", "center", "space-between", "space-around", "space-evenly", "flex-start", "flex-end" -> true
        else -> false
    }

    private fun isValidAlignItems(alignItems: String): Boolean = when (alignItems.lowercase()) {
        "start", "end", "center", "stretch", "baseline", "flex-start", "flex-end" -> true
        else -> false
    }

    private fun isValidTextOverflow(textOverflow: String): Boolean = when (textOverflow.lowercase()) {
        "clip", "ellipsis", "fade", "visible" -> true
        else -> false
    }

    private fun isValidKeyboardType(keyboardType: String): Boolean = when (keyboardType.lowercase()) {
